...
Migrate workloads in recovery-complexity waves. Every retained workload must complete backup, scratch restore, content and ownership validation, replacement-PVC restore, application startup validation, a post-cutover backup, and Miroir health checks. Old PVCs will be deleted rather than retained after successful cutover. Application PVCs remain protected even when CloudNativePG owns their PostgreSQL recovery; CloudNativePG database volumes are outside Kopiur's scope. The rebuildable `seerr-config-cache` was used for testing and then removed from permanent protection.

The original plan required three monthly Kopiur recovery points before retiring VolSync. After every workload passed backup, restore, application, and storage validation, the operator explicitly approved early retirement on 2026-08-24. VolSync manifests were archived with `git mv`; Restic repositories, credentials, and archived restore manifests remain available through at least 2026-11-22 and require a separate explicit decision before deletion.
