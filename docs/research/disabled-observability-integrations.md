...
| P4 | High | Immich server v3.1.0 | Immich metrics are enabled and port 8081 is exposed, but the complete app-template ServiceMonitor block is commented out. | No first-party Grafana dashboard was present at the pinned tag. | `kubernetes/misc/immich/app/server/release.yaml:36-43,93-108`. |
...
| P6 | High | Garage v2.3.0 | Garage's admin endpoint is exposed on 3903 and its first-party Prometheus dashboard is already declared in this repo, but no ServiceMonitor/PodMonitor scrapes the admin metrics endpoint. | Enabled: the Garage dashboard is declared; this row is a metrics-only gap. | `kubernetes/storage/garage/app/release.yaml:52-60,109-115`; dashboard at `kubernetes/observability/kube-prometheus-stack/app/grafanadashboard.yaml:139-155`. |
...
| P12 | Medium | VolSync 0.16.0 | The metrics endpoint is deliberately made unauthenticated, but there is no ServiceMonitor selecting its metrics Service. | — | `kubernetes/storage/volsync/volsync/app/release.yaml:9-12,25-30`. |
...
- **Immich v3.1.0:** first-party monitoring documentation defines the metrics port and `IMMICH_METRICS_INCLUDE`: [monitoring.md](https://github.com/immich-app/immich/blob/8aa95c67470a02a8ddedf03c2e52963af33065ff/docs/docs/features/monitoring.md).
